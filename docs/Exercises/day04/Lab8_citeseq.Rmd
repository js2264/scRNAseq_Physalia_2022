---
title: "Lab 8 - Single-cell multi-omics with CITE-seq"
output: 
...
# Objects to save.
writedir <- "~/"
Rda.RNA.path <- paste0(writedir, "citeseq_RNA.Rda")  # cbmc clustered using RNA
Rda.multi.path <- paste0(writedir, "citesq_cbmc_multi.Rda")  # cbmc clustered and ADT added as an assay
Rda.protein.path <- paste0(writedir, "citeseq_protein.Rda")  # cbmc clustered using protein
```
...
# Additional exploration: another example of multi-modal analysis (single cell ATAC-Seq)
For another nice example of multi-modal analysis, please explore this [single cell ATAC-Seq vignette](https://www.bioconductor.org/packages/devel/bioc/vignettes/cicero/inst/doc/website.html) and this [scATAC-seq data integration](https://satijalab.org/signac/articles/integrate_atac.html).

# Acknowledgements
This document is largely a tutorial from Seurat website, with some small modifications. The official vignette is available at [Using Seurat with multi-modal data](https://satijalab.org/seurat/multimodal_vignette.html). An even more sophisticated analysis of CITE-Seq data is available using [Weighted Nearest Neighbor Analysis](https://satijalab.org/seurat/articles/weighted_nearest_neighbor_analysis.html).
